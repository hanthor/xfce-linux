...
# Local equivalent:
#   just debug=1 iso-sd-boot xfce-linux
#   just plain-test-qemu xfce-linux

plain-qemu-monitor-live := "/tmp/xfce-linux-qemu-plain-live.sock"
plain-qemu-monitor-installed := "/tmp/xfce-linux-qemu-plain-installed.sock"
plain-qemu-serial-live := "/tmp/xfce-linux-qemu-plain-live-serial.log"
plain-qemu-serial-installed := "/tmp/xfce-linux-qemu-plain-installed-serial.log"
...
plain-test-qemu target installer_channel="stable":
    #!/usr/bin/bash
    set -euo pipefail
    DISK="/var/tmp/xfce-linux-plain-install-{{target}}-{{installer_channel}}.qcow2"
    SCRATCH="/var/tmp/xfce-linux-plain-scratch-{{target}}-{{installer_channel}}.img"
...
# for the same success/failure markers <target>/src/luks-unlock.py's qemu
# mode checks for, so a regression here surfaces the same way a LUKS one does.
plain-wait-boot target:
    #!/usr/bin/bash
    set -euo pipefail
